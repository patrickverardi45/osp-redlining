...
|---|---|---|---|---|
| `C:\Nova\projects\TrueLine\TrueLine_Beta` (backend/engine) | `feat/truelinev2` | **`9baa98a`** (photo byte-serving) + this doc bump | **PUSHED** to `origin/feat/truelinev2` (0/0); `origin/main` `068a279` UNTOUCHED | clean (untracked proof PNGs only) |
| `C:\Nova\projects\trueline-web-experience` (FieldRoute web) | `main` @ `52b46c5`; feature branch `feat/hector-preflight` (@ `536fb26`) still exists | prior arc MERGED via PR #6 (`52b46c5`); NEW photo-thumbnails branch `feat/field-evidence-photo-thumbnails` @ **`3927071`** PUSHED → **PR #7 OPEN** (base `main` ← that branch) | web `origin/main` **`52b46c5`** UNTOUCHED | clean (untracked `.next.prev-*` rollbacks) |
| `C:\Nova\projects\trueline-field-mobile` (field app) | `master` | `4ac00a8` | **NO remote at all** — local-only (owner-gated) | clean |
| `C:\Nova\knowledge\TrueLine-Wiki` (vault) | — | continued-113 save | no remote; large pre-existing drift | stage ONLY touched files, never `git add -A` |

Engine truth: `DETERMINISTIC_AUTO=49`, deterministic **50/58**, cold matrix **11/11** — unchanged; targeted field-evidence/upload/product-API suites **140 pass** (photo slice). **Field-evidence photo byte-serving slice PUSHED, review-in-flight:** backend `9baa98a` adds a tenant-scoped PHOTO read `GET /jobs/{id}/uploads/{upload_id}/photo` (mounted only under `TL2_FIELD_EVIDENCE_API_OPTIN`; PHOTO-kind allowlist + fixed image content-type + path containment; wrong-tenant/missing/non-PHOTO/unsafe → uniform 404; **pure read**, no lifecycle/slot/review/closeout/placement/AUTO change, snapshot-locked). Web `3927071` renders thumbnails behind the **default-OFF `NEXT_PUBLIC_TL2_FIELD_EVIDENCE_THUMBS`** (null uploadId never fetches; failed fetch → honest "Photo unavailable."; no mock). **Safe to ship dark.** **NOT live on staging** — the new route needs an owner-approved backend bounce onto `9baa98a`, and thumbnails need a web rebuild with the flag; staging still runs the prior build. The prior arc is on web `main` via PR **#6** (`52b46c5`; PR **#5** auto-resolved/superseded, `f16d88c` in `main` once). Backend `origin/main` `068a279` + mobile UNTOUCHED.
...
1. **PRs.** PR #6 (prior arc → web `main` `52b46c5`) MERGED; PR #5 auto-resolved/superseded. **PR #7 OPEN** (`feat/field-evidence-photo-thumbnails` @ `3927071` → `main`) — the opt-in photo-thumbnails slice, safe-to-ship-dark, awaiting owner review/merge; backend half `9baa98a` already pushed. **Staging photo activation (separate, owner-gated):** after PR #7 merges, bounce the backend onto `9baa98a` (flag already ON) + rebuild web with `NEXT_PUBLIC_TL2_FIELD_EVIDENCE_THUMBS=1`, then temp-job live smoke. Optional: delete the merged `feat/hector-preflight` branch.
...
4. **Camera/GPS** native modules; bore-reading plotting on the proof surface. (Photo byte-serving route + opt-in thumbnails = DONE, PR #7 pending review.)
