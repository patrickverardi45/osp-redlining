...
> Snapshot below is current as of **2026-06-21 (continued 47 — billing_summary server-computed model landed (one durable server-authoritative billing status per job; Decimal money, versioned injected cost rules, FINAL only when closeout APPROVED, +23 tests); spine upload→review→handoff→export-safety→closeout→billing; HEAD `42c6bab`; NO render-truth change; frontier 50/58)**. For the absolute-latest
...
- Product lives in `truelinev2/` (clean-room, zero old-app imports). v2 suite: **1593 passed / 4 skipped**
  (continued-47: +23 billing_summary server-computed tests). Callout-sweep e2e **31 passed / 1 skipped**.
...
- Last RENDER commit: **`c19b565`** (log3 wired + DRAWN, 49→50/58 — UNCHANGED). Engine/contract HEAD = **`42c6bab`**
  (continued-47; billing_summary server-computed — `contracts: add billing summary server-computed model`, 3 files/+1022, pushed → `origin/feat/truelinev2` = `42c6bab`; tree clean; the continued-47 docs re-canonicalization sits one commit above). Prior: continued-46 closeout `43a8501` (docs `f7e0772`); continued-45 kmz_export `2f70e22`; continued-44 handoff `2013879`; continued-43 gate `6048ef1`; continued-42 Slice 1 `2193a0e`; continued-41 `a4bf2a5` (docs/audit). Continued-41 engine-repo arc (7 commits on the continued-40 save `021561c`):
...
## Latest — continued 47 (2026-06-21): billing_summary server-computed model landed (one durable server-authoritative billing status per job; FINAL only when closeout APPROVED); NO render-truth change; frontier 50/58
...
1. **`export_package` versioned closeout packet ← recommended next (separately authorized; NOT started).**
   A stored, versioned, checksummed, REPRODUCIBLE closeout export packet that bundles ONLY trusted product outputs
   (job summary, reviewed-bore-log gate result, validated manifest handoff, served redline artifacts / durable bundle
   refs, kmz_export safety result, closeout_review status, billing_summary snapshot) — never inventing truth. Generated
   server-side, resolvable by a stable ref; footer records `generated_at`/`generated_by` + source `artifact_bundle` id +
   manifest `sha256` for reproducibility; generation gated on closeout ≥ READY_FOR_APPROVAL, share/billing-grade requires
   APPROVED. Contract §8. Builds on the continued-42→47 spine (HEAD `42c6bab`). Contract-first; generic names; no
   backend/UI/deploy. (Lane `TRUELINE_PRODUCT_EXPORT_PACKAGE_VERSIONED_CLOSEOUT`.)
