...
| `C:\Nova\projects\trueline-web-experience` (FieldRoute web) | `main` @ **`0f85d85`** (local == origin) | photo-thumbnails slice **MERGED via PR #7** (merge commit `0f85d85`, contains `3927071`); prior arc via PR #6; branches `feat/field-evidence-photo-thumbnails` + `feat/hector-preflight` still exist | web `origin/main` **`0f85d85`** (was `52b46c5`) | clean (untracked `.next.prev-*` rollbacks) |
...
Engine truth: `DETERMINISTIC_AUTO=49`, deterministic **50/58**, cold matrix **11/11** — unchanged; targeted field-evidence/upload/product-API suites **140 pass** (photo slice). **Field-evidence photo byte-serving slice is now MERGED to web `main`** (PR **#7**, merge commit `0f85d85`): backend `9baa98a` (pushed on `feat/truelinev2`) adds a tenant-scoped PHOTO read `GET /jobs/{id}/uploads/{upload_id}/photo` (mounted only under `TL2_FIELD_EVIDENCE_API_OPTIN`; PHOTO-kind allowlist + fixed image content-type + path containment; wrong-tenant/missing/non-PHOTO/unsafe → uniform 404; **pure read**, no lifecycle/slot/review/closeout/placement/AUTO change, snapshot-locked). Web thumbnails render behind the **default-OFF `NEXT_PUBLIC_TL2_FIELD_EVIDENCE_THUMBS`** (null uploadId never fetches; failed fetch → honest "Photo unavailable."; no mock) — **merged but DARK**. **NOT live on staging:** the route needs an owner-approved backend bounce onto `9baa98a`, and thumbnails a web rebuild with the flag ON; staging still runs the prior build. Prior arc on web `main` via PR **#6** (`52b46c5`; PR **#5** auto-resolved/superseded). Backend `origin/main` `068a279` + mobile UNTOUCHED.
...
1. **PRs — all merged.** PR #6 (prior arc, `52b46c5`) + PR #7 (photo thumbnails, merge commit `0f85d85`) MERGED to web `main`; PR #5 auto-resolved/superseded. **NEXT owner-gated slice = STAGING PHOTO ACTIVATION** (currently merged-but-dark): bounce the staging backend onto `9baa98a` (flag `TL2_FIELD_EVIDENCE_API_OPTIN` already ON) + rebuild staging web with `NEXT_PUBLIC_TL2_FIELD_EVIDENCE_THUMBS=1`, then temp-job live smoke (right-tenant 200 / wrong-tenant 404 / PLAN_PDF 404 / thumbnail visible; demos untouched). Optional cleanup: delete the merged `feat/hector-preflight` + `feat/field-evidence-photo-thumbnails` branches.
