...
| Provider callable raises `ProviderOutputInvalid` (malformed/refused model response, missing fixture, …), or its returned dict fails `validate_page_extraction` | `HANDWRITTEN_PROVIDER_OUTPUT_INVALID` | the `ProviderOutputInvalid` message when present (these are always deliberately-written, safe strings from in-repo provider code — never raw SDK/network text), else a fixed schema-failure message |
...
The API key itself is never part of any refusal, log line, or exception message this repo constructs —
the SDK's own exceptions may embed request context, which is exactly why `HANDWRITTEN_PROVIDER_ERROR`
reduces them to a class name before they ever reach a refusal record or a log line.

## Retry policy (Anthropic adapter only)

`anthropic_messages.py` retries a single page's Messages API call up to **2 additional times** (3 attempts
total) — **only** on 429 (rate limit), 5xx (server error), or a connection/timeout error — with a short
jittered backoff between attempts. A 4xx auth/permission error, or any other non-retryable failure, is
**never** retried; it propagates on the first attempt and the seam reduces it to `HANDWRITTEN_PROVIDER_ERROR`
(class name only). All attempts happen inside the seam's own per-page `TL2_HANDWRITTEN_BORELOG_TIMEOUT_SECONDS`
wall-clock timeout — retries do not extend it. The adapter constructs its own `anthropic.Anthropic()` client
with `max_retries=0`, so this adapter-level retry loop is the ONLY retry logic in play (no doubled retries
from the SDK's own default client-level retry behavior).
