...
span is 170'; log36 stays un-anchored, log66 is bridged-not-promoted, log59 is now seam-promoted; and the proof reuses the
...
def test_log36_un_anchored_log66_bridged_not_promoted_log59_promoted():
    assert not REC["log36"].get("endpoint_anchors")
    assert REC["log66"].get("endpoint_anchors")        # log66 bridged (source-recovered sheet 10) but NOT seam-promoted
    ...
    with pytest.raises(ValueError):
        build_seam_payload("log66", REC["log66"])      # bridged but held back from the seam
