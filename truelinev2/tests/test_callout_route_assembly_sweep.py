...
from truelinev2.proof.run_callout_route_assembly_sweep import (
    ALLOWED, ALREADY_DRAWN, BASE_CONDUIT, R_COMPLETE,
)
from truelinev2.extract.structure_position import (
    BRENHAM_CONDUIT_LAYERS, BRENHAM_LATERAL_CONDUIT_LAYERS,
)

CROSS_SHEET_TARGETS = ("log11", "log47", "log58", "log67")   # two sheet-local legs
...
@pytest.mark.skipif(os.environ.get("TL2_TRY_DRAW_E2E") != "1",
                    reason="heavy end-to-end (binds + renders 4 cross-sheet bores); set TL2_TRY_DRAW_E2E=1 to run")
def test_sweep_renders_four_new_logs_end_to_end():
    import json
