r"""M8.7/M8.8 -- station-axis interval containment / path-walk solver (pure core).

Proven proof-first as M8.7 (proof/run_station_axis_interval_containment) and
re-homed into match/ for the default-OFF M8.8 wiring (TL2_STATION_AXIS_INTERVAL_
OPTIN) -- the M8.2k -> M8.2l / M8.5 precedent. The original module path
(proof/station_axis_interval_containment) re-exports this one.
...
class StationAxisContext:
    """Everything the engine's default-OFF M8.8 retry needs, prebuilt by the
    caller (RedlineService when ``Settings.station_axis_interval_optin`` is
    True): per-sheet axis tick values and the axis callout universe over the
    bore's referenced sheets +-1 (a path may CONTINUE onto an adjacent sheet;
    the print-scope guard still forbids claims living entirely off the refs)."""
    ticks_by_sheet: Dict[int, Tuple[float, ...]]
...
def solve_interval_path(*, bore_id: str, bore_start_ft: float, bore_end_ft: float,
                        span_ft: float, ticks_by_sheet: Dict[int, Sequence[float]],
                        callouts: Sequence[Callout],
                        sheet_refs: Sequence[int] = ()
                        ) -> Tuple[Dict[str, object], Optional[List[Callout]]]:
    """The full M8.7 proof for one bore. Returns ``(verdict_dict, chain)`` where
    ``chain`` is the proven interval Callouts ONLY on READY (else None). NEVER a
    ...
    if not end_ticks and not holding_clusters:
        ...
        return ({**base, "result": TICK_NOT_FOUND}, None)
    ...
    if rivals:
        ...
        return ({**base, "result": FRAME_CONFLICT}, None)
    if not ladder or ladder[0].lo > expected_start + START_TOL_FT:
        ...
        return ({**base, "result": DISCONTINUITY}, None)
    ...
    if walk["chain"] and walk.get("n_holding_intervals", 0) > 1:
        ...
        return ({**base, "result": PICK_CARD}, None)
    if not walk["complete"]:
        if not walk["chain"]:
            ...
            return ({**base, "result": FOOTAGE_MISMATCH}, None)
        base["named_missing_relationship"] = (
    ...
    base["end_tick_delta_ft"] = (round(min(abs(v - bore_end_ft) for _s, v in end_ticks), 2)
                                 if end_ticks else None)
    return ({**base, "result": READY, "caveats": caveats, "review_gated_only": True},
            list(walk["chain"]))


def prove_interval_path(**kwargs) -> Dict[str, object]:
    """Proof-harness convenience: the verdict dict only (JSON-serializable)."""
    proof, _chain = solve_interval_path(**kwargs)
    return proof
